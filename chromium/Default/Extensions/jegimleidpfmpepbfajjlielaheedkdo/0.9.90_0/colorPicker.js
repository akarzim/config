 ...
 *
 * @version 0.9 [Google Chrome™ edition]
 * @license ©2010 by Peter Dematté
 * @author  Peter Dematté, http://dematte.at/colorPicker
 * @created 2010-11-03
 * @updated 2011-07-13
 * @status  beta
 * @link    http://dematte.at/colorPicker
 * inspired by johndyer.name, quirksmode.org
 *
*/
function colorPicker(e){
	// attribute definitions
	var	cP = colorPicker, // for now to make code shorter
	mode = cP.mode||'B', // ['H', 'S', 'V', 'R', 'G', 'B'] initial colorMode
	size = cP.size||4, //[1, 2, 3, 4] initial size from XXS to L
	allowResize = cP.allowResize != undefined?cP.allowResize:true, //[true, false] switch to allow/disallow resizing of app
	allowClose = cP.allowClose != undefined?cP.allowClose:true, //[true, false] switch to allow/disallow closing of app with 'x' in right top corner
	allowDrag = cP.allowDrag != undefined?cP.allowDrag:true, //[true, false] switch to allow/disallow dragging app
	expColor = cP.expColor != undefined?cP.expColor:true, //[true, false] switch for exporting colors to background of initial object
	expHEX = cP.expHEX != undefined?cP.expHEX:true, //[true, false] switch for exporting color-value to initial object (as value or text)
	offsetX = cP.offsetX||0, //[int] x-offset in pixels relative to initial object
	offsetY = cP.offsetY||3, // [int] y-offset in pixels relative to initial object
	orientation = cP.orientation||['bottom','left'], //['left||right','bottom||top'] initial xy-position relative to initial object
	parentObj = cP.parentObj||null, // [HTMLObject] initial parent of colorPicker
	parentXY = cP.parentXY||'', // [right:12px;top:12px] initial style (coordinates,...)
	color = cP.objs||[204,0,0], // initial color in common web-formats
	difPad = cP.difPad||2, // LR padding/border/left of contrast / color difference bar
	rSpeed = cP.rSpeed!= undefined?cP.rSpeed:15, // rendering speed; 0 renders right after mousemove
	cookieLife = cP.cookieLife||0, // rendering speed; 0 renders right after mousemove
	...
	CP=(!cP)?function(obj) { // building the colorPicker / setting up events and methodes
		...
		if (IE) testPix.src=_IE+'_horizontal'; if (!IE || IE8 || testPix.height==1) CSSTxt = CSSTxt.replace(/url\(_[a-z]*\.png\);*/g,"") + '.cPSkinC01,.cPSkinC02,.cPSkinC03,.cPSkinC04,.cPSLCB,.cPSLCW,.cPSRCLB,.cPSRCRB,.cPSRCLW,.cPSRCRW,.cPM0,.cPClose,.cPResize,.cPinpDrag,.cPinpDragOn{background-image:url('+(IE&&!IE8?_IE+'_icons':_nonIE+_icons)+')!important}.cPSL2R,.cPSL3R,.cPSL2G,.cPSL3G,.cPSL2B,.cPSL3B{background-image:url('+(IE&&!IE8?_IE+'_patches':_nonIE+_patches)+')!important}.cPSR1R,.cPSR2R,.cPSR3R,.cPSR4R,.cPSR1G,.cPSR2G,.cPSR3G,.cPSR4G,.cPSR1B,.cPSR2B,.cPSR3B,.cPSR4B,.cPSR3H,.cPSR2V,.cPSL3H,.cPSL2S,.cPSR4V,.cPSL3S,.cPSL2V,.cPSR2S{background-image:url('+(IE&&!IE8?_IE+'_vertical':_nonIE+_vertical)+')!important}.cPSL2H,.cPSL1S,.cPSL1V{background-image:url('+(IE&&!IE8?_IE+'_horizontal':_nonIE+_horizontal)+')!important}';
		...
		cP.cPCD1  = (cP.cPCD1  = /\.cPCD1\s*{(.*?)}/.exec(CSSTxt)) ? cP.cPCD1[1]+'!important' : '';
		cP.cPCD2  = (cP.cPCD2  = /\.cPCD2\s*{(.*?)}/.exec(CSSTxt)) ? cP.cPCD2[1]+'!important' : '';
		...
		cP.onmousedown = function(e){
			...
			if (document.funcCache) stopDrag(); // just in case somebody left the browser...
			...
			if ((!obj.className.search(/(cP(HSB|RGB|CMYK|HEX|Panel|Skin)|(.*?\snoB))/) ||
			   (obj.name && !obj.name.indexOf('cPI') && obj.readOnly)) && allowDrag) { // drag app
				if (colorPicker.onDrag) colorPicker.onDrag(); // faster way???
				mousePos = getMousePos(e,getOrigin(cP));
				addEvent(document, 'mousemove', function(e){
					var mP = getMousePos(e,[0,0]);
					if (IE && (!this.skip?this.skip=1:this.skip++)%2) return false; // delay for IE 
					cP.cPSkins.cssText = 'position:fixed;left:'+(mP[0]-mousePos[0]-document.body.scrollLeft)+'px;top:'+(mP[1]-mousePos[1]-document.body.scrollTop)+'px';
					return false});
				...
				if (iPhone) {nCN = oCN == 'cPSkin'?' S':oCN == 'cPSkin S'?' S XS':oCN == 'cPSkin S XS'?' S XS XXS':''; resizeWin(nCN)}
				else addEvent(document, 'mousemove', function(e){
					...
					if (cP.className != 'cPSkin' + nCN + ' cPX') resizeWin(nCN);
					return false});
				...
				if (xy) cP.cPSL4.className = 'cPSL4 cPSL4NC cPX';
				addEvent(document, 'mousemove', function(e){doDrag(e,origin,xy);return false});
	...
	initCp = function(rgb) {
		...
		for(var n=0,nob='RGB'.split('');n<nob.length;n++) if (CP.modeRGB[nob[n]]) cP['cPBR'+nob[n]].className = 'cPBR'+nob[n]+' '+'bUp cPX';
		difWidth = getStyle(cP.cPCTRT.parentNode,'width').replace('px','')-difPad; // max width of contrast/color bar
		CP.iCtr = getBrightness(CP.CB2Color); cCtr=null; // of the current saved color !!!
		...
		for(var n=1, m='L';n<=3;n++) { // preset all classNames in all sliders
			cP['cPS'+m+n].className = 'cPS'+m+n+CP.mode+(CP.modeRGB[CP.mode]&&n>1&&m=='R'?' cPhide':'')+' cPX';
			if (n>2 && m=='L') {m='R'; n=0}}
	...
	doRender = function(xy,yz) { // function for pure rendering. No rendering elsewhere!!
		...
		if (xy) { // left slider -> right
			...
			else if (CP.mode != 'H' && !CP.modeRGB[CP.mode]) cP.cPSR2.className = 'cPSR'+(2+a)+CP.mode+' cPOP'+Math.round((CP.xyz[a]-CP.xyz[b])/(255-CP.xyz[b])*100 || 0)+' cPX';
			if (CP.mode != 'H' && !CP.modeRGB[CP.mode]) cP.cPSR3.className = 'cPSR4'+CP.mode+' cPOP'+Math.round(Math.abs(c-CP.xyz[b])/2.55)+' cPX';
		...
		if (!xy || yz) { // right slider -> left
			...
			else cP.cPSL3.className = 'cPSL3'+CP.mode+' cPOP'+(100-Math.round(CP.xyz[2]/2.55))+' cPX';
		...
		if (cCtr !== nCtr) {
			if (nCtr) {if (expColor) cP.cObjs.color = '#222'; cP.cPSLC.className = 'cPSLCB cPX'; CP.cPM0CN = 'cPM0 cPM0B cPX';
				if (CP.mode != 'H' && !CP.modeRGB[CP.mode]) {cP.cPSRCL.className = 'cPSRCLB cPX';
					if (xy) cP.cPSRCR.className = 'cPSRCRB cPX'}
			}	else {if (expColor) cP.cObjs.color = '#ddd';cP.cPSLC.className = 'cPSLCW cPX'; CP.cPM0CN = 'cPM0 cPX';
				if (CP.mode != 'H' && !CP.modeRGB[CP.mode]) {cP.cPSRCL.className = 'cPSRCLW cPX';
					if (xy) cP.cPSRCR.className = 'cPSRCRW cPX'}
			}
		} cCtr = nCtr;
		if (!xy || yz) { // only right
			if (CP.modeRGB[CP.mode]) {
				...
				if (crCtr !== nrCtr){ // RGB special mode
					if (nrCtr && CP.mode == 'G') {cP.cPSRCL.className = 'cPSRCLB cPX'; if (xy) cP.cPSRCR.className = 'cPSRCRB cPX'; 
					} else {cP.cPSRCL.className = 'cPSRCLW cPX'; if (xy) cP.cPSRCR.className = 'cPSRCRW cPX'}}
			} else if (CP.mode == 'H') { // HSB_H mode extra rules
				nrCtr = getBrightness(tmpHSV || HSV2RGB(CP.hsv[0],255,255)) > 128;
				if (crCtr !== nrCtr) {if (nrCtr) cP.cPSRCL.className = 'cPSRCLB cPX'; else cP.cPSRCL.className = 'cPSRCLW cPX'}
			} crCtr = nrCtr;
		}
	...
	stopDrag = function(){
		...
		cP.cPSL4.className = 'cPSL4 cPX'; // cursor to normal
		cP.cPResizers.cssText='';
		// cP.cObj.osXY = 'position:fixed;left:'+cP.style.left+';top:'+cP.style.top+';'; // save position
		if (cP.inp) cP.inp.className = 'cPX';
	},
	...
	resizeWin = function(nCN) {
		cP.className = 'cPSkin' + nCN + ' cPX'; sX=(nCN)?2:1; sZ=(nCN==' S XS XXS')?2:1; xyzCorr=(sX>1?128:0)+(sZ>1?64:0);
		...
		cP.cPRGB.className = (cPM=='RGB' || !nCN) ? 'cPRGB cPX' : 'cPhide'; cP.cPHSB.className = (cPM=='HSV' || !nCN) ? 'cPHSB cPX' : 'cPhide';
		setCookie('size',nCN==' S XS XXS'?1:nCN==' S XS'?2:nCN==' S'?3:4);
		if (colorPicker.onResize) colorPicker.onResize(nCN==' S XS XXS'?1:nCN==' S XS'?2:nCN==' S'?3:4);
	},

	toggleCp = function(onOff,obj) { 
		var cPPS;
		if (onOff) {cP.cPSkins.display = 'none'; if (colorPicker.onClose) colorPicker.onClose()}
		else {if (colorPicker.onOpen) colorPicker.onOpen();
			if (!parentObj) {
			...
			*/
			cP.cPSkins.cssText = 'position:fixed;right:12px;bottom:12px;';
	...
	init = function() {
		var obj = parentObj || e.target || e.srcElement, ca = document.cookie.split(';'), n, c;
			...
			if (cP.WEBS1) cP.WEBS1=null; // kill the memory of webSmart/Save button
			cP.cObjs = obj.style;
			if (obj.value) CP.valPrefix = /(#*)/.exec(obj.value)[0]; // wether you have an # or an ## (Cold Fusion) or none
...
/*
 * colorPicker [Google Chrome™ edition]
 *
 * @version 0.7
 * @author  Peter Dematté, http://dematte.at/colorPicker
 * @created 2011-01-02
 ...
 * @status  beta
 *
 * This is kind of quick and dirty,... so please don't mind the coding ;o)
*/
var cP = colorPicker,
    cPToggle = false,
		cPDummyCSS = document.createElement('style'),
		_DummyEl = document.createElement('div'),
    cPCSS = document.createElement('style'), cPCSSText,
    cPRanger = document.createElement('div'),
    cPDOM = document.createElement('div'), cPDomDragging,
		cPHover = document.createElement('div'), cPHoverEl, cPHoverStyle, cPHoverInnerStyle, cPHoverTimer, 
    doCapture=true,
		preventCursor = false,
		colorPicking = false,
    cPRange = cPRange || 11,
		cPMode = cPMode != 'undefined' ? cPMode : 1,
		cPSize = cPSize != 'undefined' ? cPSize : 4,
		scanner = ['color','background-color','border-top-color','border-right-color','border-bottom-color','border-left-color']; // ,'border-color'
		optsN = document.body.getElementsByTagName('*'), opts={BASE:1,BR:1,NOSCRIPT:1,PARAM:1,SCRIPT:1,STYLE:1,LINK:1}, optsCol=[''], optsCSS=[];

// collect Tags in document
for(var n=0, m=optsN.length; n<m; n++) if (!opts[optsN[n].tagName]) {optsCol.push(optsN[n].tagName); opts[optsN[n].tagName] = true} // .toLowerCase()
for(var n=0, m=optsCol.length, opts=[]; n<m; n++) {
	opts.push('<option class="tagSelectorOption" value="'+optsCol[n]+'">'+optsCol[n]+'</option>');
	if (optsCol[n]) optsCSS.push(optsCol[n]+':not(.cPX){}');
}
opts = opts.sort().join(''); optsCSS = optsCSS.sort().join(''); optsCol=optsN=null;

// dummyStylesheet for element.CSS manipulation
cPDummyCSS.setAttribute('type','text/css');
cPDummyCSS.setAttribute('id', 'cPDummyCSS');
cPDummyCSS.appendChild(document.createTextNode(optsCSS));
document.getElementsByTagName('head')[0].appendChild(cPDummyCSS);
cPDummyCSS = document.styleSheets['cPDummyCSS'];

// elements higlighting mask
cPHover.setAttribute('id', 'cPHover');
cPHover.innerHTML = '<div id="cPHoverInner"></div>';
cPHover = document.body.parentElement.appendChild(cPHover);
cPHoverStyle = cPHover.style; cPHoverStyle.display = 'none';
cPHoverInnerStyle = cPHover.firstChild.style;

// dummyElement to hold new Elements for CSS manipulation
_DummyEl.setAttribute('id', '_DummyEl');
_DummyEl.style.cssText = 'display:none!important';
_DummyEl = document.body.parentElement.appendChild(_DummyEl);

cPCSSText = 'body *{}'+
	'#cPRanger{position:fixed;border:1px solid #000;opacity:.7;background-color:transparent;z-index:2147483646}'+
	'#cPHover{border:1px solid #3f56b4;background-color:rgba(127,186,242,.2);position:absolute;z-index:2147483646}'+
	'#cPHoverInner{border:1px solid #3f56b4;background-color:rgba(127,186,242,.4);position:absolute}'+
	'#cPDOM{width:auto;border:1px solid #444;border-radius:5px;background-color:#444;z-index:2147483647;position:fixed;text-align:left;font-family:Geneva,Arial,Helvetica,sans-serif;margin-right:10px}'+
	'#cPDOM *{color:#444;width:auto;font-family:"Courier New", Courier, monospace ;font-size:12px;font-weight:normal;cursor:default}'+
	'#cPXDomExplHead{font-family: Arial, Helvetica, sans-serif;font-weight:bold;padding:1px;line-height:17px;background-color:#ddd;border:1px solid #ddd;border-radius:2px;border-bottom-right-radius:0;border-bottom-left-radius:0;margin:2px 2px 0px 2px}'+
	'#cPCoverDiv span{background-color:#efefef;display:inline-block;margin:1px 0 2px;padding:0px 6px;color:#444;font-size:11px;line-height:22px;font-family:Arial;border-left:1px solid #bbb}'+
	'#cPCoverDiv span:hover{background-color:#d4d4d4}'+
	'#cPCoverTable{width:auto;border:0;margin:0;padding:0}'+
	'#cPCoverTable tr:hover{width:auto;height:auto;background-color:#f5f5f5}'+
	'#cPCoverTable td{width:auto;height:16px;margin:0px;padding:1px;border:0;white-space:nowrap;line-height:16px;vertical-align:middle}'+
	'#cPCoverTable label{float:left;height:auto;width:100%}'+
	'#cPCoverDiv .Hi,#cPCoverDiv .Hi:hover{color:#eee;background-color:#3879d9}'+
	'#cPXSpyGlass{vertical-align:middle;margin:0 0 4px;height:13px;width:13px;padding:0}'+
	'.cPX_Input{float:left;margin:3px}'+
	'.tagSelectorOption{background-color:#eee}'+
	'.cPDELETE{margin-left:3px;padding:0 3px;border:1px solid #888;border-radius:2px}'+
	'.cPDELETE:hover{background-color:#f66}'+
	'.cPXClose{float:right;padding:0 4px;line-height:15px;font-weight:bold;border:1px solid #777;border-radius:2px}'+
	'.cPXClose:hover{background-color:#f66}'+
	'#cPFindContent{position:relative;background-color:#fff;margin:1px 2px 0px 2px;padding:2px 3px 3px 0}'+
	'#cPRangerCover{position:absolute;top:-15px;right:-15px;bottom:-15px;left:-15px;cursor:url(\'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQI12P4//8/MwAI/wMBbrqo4gAAAABJRU5ErkJggg==\'),crosshair;background:url(\'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAA0AAAANCAYAAABy6+R8AAAAK0lEQVQoz2P4//8/AzYMBP9xyg28pv8QMNCaQG6HYTSAIoCsbrD7aXCnCAB1su0h62z+8AAAAABJRU5ErkJggg==\') no-repeat 50% 50%}'+
	'#cPCoverDiv{margin:0 2px;border-radius:5px;overflow:hidden;border-top-right-radius:0;border-top-left-radius:0}';
cPCSS.appendChild(document.createTextNode(cPCSSText.replace(/;/g,'!important;').replace(/\}/g,'!important}').replace(/\!important\;base64/g,';base64').replace(/\{\!important\}/g,'{}')));
cPCSS.setAttribute('type','text/css');
cPCSS.setAttribute('id', 'cPCSS');
document.getElementsByTagName('head')[0].appendChild(cPCSS);
cPCSS = document.styleSheets['cPCSS'];


// cursor for color picker
cPRanger.setAttribute('id', 'cPRanger');
cPRanger.innerHTML = '<div style="position:absolute;top:-1px;right:-1px;bottom:-1px;left:-1px;border:1px dotted #fff!important;background-color:transparent!important"></div><div id="cPRangerCover"></div>';
cPRanger = document.body.parentElement.appendChild(cPRanger);

// DOM Color-Explorer
cPDOM.setAttribute('id', 'cPDOM');
cPDOM.setAttribute('class', 'cPX');
cPDOM.style.cssText = 'display:none;left:12px;top:12px;';
cPDOM = document.body.parentElement.appendChild(cPDOM);
cPDOM.cPOldObj = document;
cPDOM.onmouseover = cPDOM.onclick = hlObj;

// redefine colorPicker on/off/drag events
cP.onClose = function() {cPToggle = false; chrome.extension.sendRequest({cPOff: true}); chListener();};
cP.onOpen = function() {cPToggle = true; chrome.extension.sendRequest({cPOff: false}); document.addEventListener('mousemove', doPicker, true); chListener(cPMode)};
cP.onDrag = function() {doCapture=true;cPDOM.style.setProperty('z-index','2147483646','important')};
cP.onResize = function(size) {cPSize = size; chrome.extension.sendRequest({cPSize: size})};
// set some parameters for colorPicker
cP.parentObj = document.body.parentElement;
cP.expColor = false;
cP.cookieLife = 365; // one year
cP.size = cPSize;
// and go!
cP(event);
// protecting the colorPicker from CSS
var allCPElms = cP.cP.getElementsByTagName('*');
for(var n = allCPElms.length;n--;) allCPElms[n].className = allCPElms[n].className+' cPX';
// hello background! I'm initialized!
chrome.extension.sendRequest({toggle: false});
chListener (cPMode);

document.addEventListener('click', function(e) { // sorry, but chrome.tabs.captureVisibleTab is a little weird...
	if (!cPToggle || e.which == 3) return false;
	if (checkForDialogues(e.target)) {e.stopPropagation(); e.preventDefault()}
}, true);
																																										
document.addEventListener('mouseup', function(e) {
	var obj = e.target, parent = obj, parentColl = [];
	// better make a onColorPicked event for colorPicker instead of the next lines.....!!!!!!!
	// if (((obj.className && obj.className.indexOf('cP') == 0 && obj.className.indexOf('cPX') != 0) || (obj.name && obj.name.indexOf('cP') == 0 && obj.name.indexOf('cPX') != 0)) && cPDOM.style.display != 'none') {
		if (colorPicking  && cPDOM.style.display != 'none') {
		if (!cPDOM.tagSelected) writeElements(cPDOM.elmBuffElm || cPDOM.cPCurrDOMObj,true);
		else writeElements(_DummyEl.children(0),true,document.getElementById('tagSelector').selectedIndex-1);
		// return false;
	}
	if (obj == document.getElementById('cPXSpyGlass')) {cPDOM.style.display='none'; cP.exportColor = null}
	if (obj == document.getElementById('cPDOMClose')) cPDOM.style.display='none';
  if (!cPToggle || e.which == 3 || colorPicking) {colorPicking = false; return false}
	if (checkForDialogues(obj)) {
		e.stopPropagation(); e.preventDefault();
		if (cPMode) chrome.extension.sendRequest({x:e.clientX, y:e.clientY, capture:doCapture, win:[window.innerWidth,window.innerHeight]}); // colorPicking
		else { // element highlighting
			cPHoverStyle.display = 'none';	obj = document.elementFromPoint(e.x, e.y); cPHoverStyle.display = ''; parent = obj; doHiglight(e,parent);
			if (cPHoverTimer) cPDOM.cPOldObj = doHiglight(e,obj);
			while (parent) {
				parentColl.push(parent.tagName.toLowerCase()+(parent.id ? '#'+parent.id : parent.className ? '.'+parent.className.replace(/\s/g, '.') : ''));
				parent = parent.parentElement;
			}
			parent = ['<div class="cPX" id="cPXDomExplHead"><div id="cPDOMClose" class="cPX cPXClose">X</div><div class="cPX" style="float:right!important;font-family:Arial,Helvetica,sans-serif!important;font-weight:bold!important">select tag: <select class="cPX" style="height:19px!important;margin-top:-1px!important;border:1px solid #aaa!important;background-color:#eee!important;color:#333!important" id="tagSelector">'+opts+'</select>&nbsp;<label for="cPXimportant" class="cPX" style="font-weight:bold!important;"><input id="cPXimportant" style="vertical-align:middle;margin:0 3px 3px!important" class="cPX" name="cPXimportant" type="checkbox" value="" />!imp</label>&nbsp;&nbsp;</div>&nbsp;&nbsp;DOM Color-Explorer</div>'];
			parent.push('<div id="cPFindContent" class="cPX"></div><div id="cPCoverDiv" class="cPX">');
			parent.push('<span class="cPX" style="border:0">&nbsp;<img id="cPXSpyGlass" class="cPX" align="top" width="13" height="13" src="data:image/png;base64," />&nbsp;</span>');
			for (var n=parentColl.length;n--;) parent.push('<span '+(!n?'class="cPXtd elm_'+n+' Hi"':'class="cPXtd  elm_'+n+'"')+'>'+parentColl[n]+'</span>');
			cPDOM.innerHTML = parent.join('')+'</div>';
			cPDOM.elmBuffElm = null; cPDOM.cPCurrDOMObj = null; cPDOM.elmBuffer = null;
			document.getElementById('tagSelector').addEventListener('change', function() {doTags(this)}, true);
			writeElements(obj); window.clearTimeout(cPHoverTimer); cPHoverTimer = window.setTimeout(function(){cPHoverTimer = false}, 2000);
		}
		return false;
	}
}, true);
...
document.addEventListener('mousedown', function(e) {
	var obj = e.target;
	if (obj.tagName.toLowerCase() == 'html') return; // scrollbar!!!!
	if (!cPToggle || e.which == 3) return false;
	if (obj == cPDOM.firstChild) { // drag DOM Color-Explorer
		e.preventDefault();
		cPDOM.style.setProperty('z-index','2147483647','important');
		cPDOM.XY = [e.clientX-parseInt(cPDOM.style.left),e.clientY-parseInt(cPDOM.style.top)];
		cPDomDragging = true;
		document.addEventListener('mousemove', dragCpDom, true);
	}	else if ((obj.className && obj.className.indexOf('cP') == 0 && obj.className.indexOf('cPX') != 0) || (obj.name && obj.name.indexOf('cP') == 0 && obj.name.indexOf('cPX') != 0)) colorPicking = true;
	else if (doCapture && cPMode) {
		preventCursor=true;
		cPRanger.style.setProperty('opacity', '.01','important');
		cPRanger.childNodes[1].style.cursor='wait';
		cP.cP.style.display='none';
		e.preventDefault();
	}	else if (obj.className.indexOf('cPX') != 0) cPDOM.tagSelected = false;
	return false;
}, true);

window.addEventListener('resize', function() {doCapture=true}, true);
window.addEventListener('scroll', function() {doCapture=true}, true);
document.addEventListener('keydown', function(e) {if (e.which === 27 && cPToggle) cP(event)}, false);
document.addEventListener('mouseout', function(){cPHoverStyle.display = 'none'}, false);
document.addEventListener('mouseup', function(e){document.removeEventListener('mousemove', dragCpDom, true); cPDomDragging = false}, true);

function writeElements(obj,swsw,skipper) { // for DOM Color-Explorer
	var iht, scan;
	if (!swsw) cPDOM.cPCurrDOMObj = obj;
	if (cPDOM.tagSelected) cPDOM.cPCurrDOMObjSel = obj;
	if (!cPDOM.colorBuffer) cPDOM.colorBuffer = 0;
	iht = '<table class="cPX" id="cPCoverTable" border="0" cellspacing="0" cellpadding="0"><tbody>';
	for(n=0, m=scanner.length, na='', naIO=0, curColor='', alpha=[]; n<m; n++) {
		if(n>1 && !parseInt(getStyle(obj,scanner[n].replace('color','width')))&& skipper == undefined) {if (cPDOM.colorBuffer == n) cPDOM.colorBuffer = 0;continue;}
		scan = getStyle(obj,scanner[n]);
		curColor = X2HEX(scan);
		alpha = scan.split(',');
		alpha = (alpha[3]) ? 'A:'+alpha[3].replace(')','').replace(' ','')+'&nbsp;' : null;
		na = obj.style.cssText || '';
		naIO = na.indexOf(scanner[n].replace(' ',''));
		iht += '<tr><td class="cPX"><label class="cPX" for="Color_'+n+'" style="display:block;width:100%!important;line-height:17px!important"><input class="cPX_'+n+' cPX_Input" id="Color_'+n+'" '+(n==cPDOM.colorBuffer || !n?'checked="checked"':'')+' name="cPX_Color" type="radio" value="" />'+scanner[n]+':</label></td><td class="cPX">'+(scan ? '#'+curColor+'<div class="cPX" style="border:2px solid #666;width:12px!important;height:12px!important;float:right;margin:0 4px;background-color:'+scan+'"></div>':'&nbsp;')+'</td><td class="cPX">'+(alpha?alpha:'')+'</td><td class="cPX" style="padding: 0 3px 2px 1px">'+(scan?(naIO!=-1 && na.substr(naIO-1,1) != '-'?'element.style':'CSS/inherited'):'')+'</td><td class="cPX"><span class="cPX cPDel'+n+' cPDELETE">del<span></td></tr>';
	}
	iht += '</tbody></table>';
	document.getElementById('cPFindContent').innerHTML = iht;
	cPDOM.style.display='';
	addEvtCP(obj,cPDOM.colorBuffer||0,skipper);
}

function hlObj(e) { // eventDelegation for cPDOM
	var obj = e.target, eType = e.type, objN = obj.className, elm_x = objN.indexOf('elm_'),
	    tagSelector = document.getElementById('tagSelector'), tagSelectorStyle = tagSelector.style;
	if (colorPicking || cPDomDragging) return; //NEW!!!
	if (eType == 'click') {
		if (obj.tagName.toLowerCase() == 'label' || obj.tagName.toLowerCase() == 'select') return;
		if (objN.indexOf('cPX cPDel') == 0) { // delete Style
			objN = objN.substr(9,1); cPDOM.colorBuffer = +objN;
			obj = cPDOM.elmBuffElm || cPDOM.cPCurrDOMObj;
			if (cPDOM.tagSelected) {
				cPDummyCSS.rules[cPDOM.tagSelected-1].style[scanner[cPDOM.colorBuffer||0]] = '';
				_DummyEl.children(0).style.removeProperty(scanner[cPDOM.colorBuffer||0]);
				writeElements(_DummyEl.children(0),true,tagSelector.selectedIndex-1);
			} else {obj.style.removeProperty(scanner[cPDOM.colorBuffer||0]); writeElements(obj,true)}
			return false;
		} else if(objN.indexOf('cPX_') == 0) { // change color target
			objN = objN.substr(4,2); cPDOM.colorBuffer = +objN;
			obj = cPDOM.elmBuffElm || cPDOM.cPCurrDOMObj;
		} else if (elm_x > 0) { // change element in DOM tree
			cPDOM.tagSelected = false;
			tagSelector.options[0].selected = 'selected';
			tagSelectorStyle.setProperty('background-color','#eee','important');
			tagSelectorStyle.setProperty('color','#333','important');
			var buttons = obj.parentElement.children; buttonsC = buttons.length, oldBuffer = cPDOM.elmBuffer || 0;
			// do action
			objN = objN.substr(elm_x+4,2); cPDOM.elmBuffer = +objN;
			obj = cPDOM.cPCurrDOMObj; 
			for (;objN--;) obj = obj.parentElement; cPDOM.elmBuffElm = obj;
			writeElements(obj,true);
			buttons(buttonsC-1-oldBuffer).className = buttons(buttonsC-1-oldBuffer).className.replace(' Hi','');
			buttons(buttonsC-1-cPDOM.elmBuffer).className = buttons(buttonsC-1-cPDOM.elmBuffer).className+' Hi';
		} else obj = null;
		if (obj) addEvtCP(obj,cPDOM.colorBuffer||0,cPDOM.tagSelected-1);
	} else if (eType == 'mouseover') {
		if(!elm_x) return;
		objN = objN.substr(elm_x+4,2);
		obj = cPDOM.cPCurrDOMObj;
		for (;objN--;) obj = obj.parentElement;
		// cPDOM.cPOldObj.style.removeProperty('outline');
		// obj.style.outline = '1px solid rgba(0,0,255,0.75)';
		// cPDOM.cPOldObj = obj;
		cPDOM.cPOldObj = doHiglight(e,obj);

	}
}

function doTags(obj) {
	var el = document.getElementById('cPCoverDiv').getElementsByTagName('span');
	el(el.length-(cPDOM.elmBuffer||0)-1).className = el(el.length-(cPDOM.elmBuffer||0)-1).className.replace(' Hi','');
	
	obj.style.setProperty('background-color','#3879D9','important');
	obj.style.setProperty('color','#eee','important');

	_DummyEl.innerHTML = '<'+obj.value+'></'+obj.value+'>';
	cPDOM.tagSelected = obj.selectedIndex;
	writeElements(_DummyEl.firstChild,true,obj.selectedIndex-1);
}

function addEvtCP (obj,prop,ruleID) {
	cP.exportColor = null;
	cP.importColor('#'+X2HEX(getStyle((cPDOM.tagSelected ? cPDOM.cPCurrDOMObjSel : obj),scanner[prop])));
	cP.exportColor = cPDOM.tagSelected ?
		// function() {cPDummyCSS.rules[ruleID].style[scanner[prop]] = 'rgb('+cP.CP.rgbRND+')'}:
		function() {if (!document.getElementById('cPXimportant').checked) cPDummyCSS.rules[ruleID].style.removeProperty(scanner[prop]); cPDummyCSS.rules[ruleID].style.setProperty(scanner[prop], 'rgb('+cP.CP.rgbRND+')',document.getElementById('cPXimportant').checked?'important':'')}: // [scanner[prop]] = 'rgb('+cP.CP.rgbRND+')'}:
		function() {obj.style.setProperty(scanner[prop], 'rgb('+cP.CP.rgbRND+')','important')};
	
}

function dragCpDom(e) { // drag DOM Color-Explorer
	cPDOM.style.cssText='left:'+(e.clientX-cPDOM.XY[0])+'px;top:'+(e.clientY-cPDOM.XY[1])+'px';
}


function doPicker(e) { // document.mousemove actions
	var obj = e.target, objS = obj.style, objName = obj.name, objCName = obj.className, objXY;
	if (colorPicking || cPDomDragging) return; //NEW!!!
	if (cPMode) {
		if (!checkForDialogues(obj)) {cPRanger.style.display = 'none'; return}// ???????? better
		cPRanger.style.cssText = 'opacity:'+(doCapture && preventCursor ? '.01':'1')+'!important;left:'+(e.clientX-(cPRange-1)/2-1)+'px;top:'+(e.clientY-(cPRange-1)/2-1)+'px;width:'+cPRange+'px;height:'+cPRange+'px';
		if (doCapture && preventCursor) cPRanger.childNodes[1].style.cssText = 'cursor:wait!important';
		return false;
	} else {
		cPHoverStyle.display = 'none'; obj = document.elementFromPoint(e.x, e.y); objName = obj.name; objCName = obj.className; cPHoverStyle.display = '';
		if (obj == cPDOM.cPOldObj) return;
		if ((objCName.indexOf('cP') != 0 && !objName) || (objName && objName.indexOf('cP') != 0)) {
			// cPDOM.cPOldObj.style.removeProperty('outline');
			// obj.style.outline = '1px solid rgba(0,0,255,0.75)';
			// cPDOM.cPOldObj = obj;
			if (!cPHoverTimer) cPDOM.cPOldObj = doHiglight(e,obj);
		} else if (objCName && objCName.indexOf('cPXtd') != 0) {
			// cPDOM.cPOldObj.style.removeProperty('outline');
			cPHoverTimer = false;
			cPHoverStyle.display = 'none';
			cPDOM.cPOldObj = document;
		} 
	}
	return false;
...
/* element highlighter */
function doHiglight(e,obj) {
	var el, space, bord, box, db = document.body;
	if (!obj) {
		cPHoverStyle.display = 'none';	el = document.elementFromPoint(e.x, e.y);
	} else el = obj;
		// if (el === cPHoverEl) {cPHoverStyle.display = ''; return};
		// if (!checkForDialogues(e.target)) {cPHoverStyle.display = 'none'; return}
	cPHoverEl = el;
	space=[parseInt(getStyle(el,'margin-top')),parseInt(getStyle(el,'margin-right')),parseInt(getStyle(el,'margin-bottom')),parseInt(getStyle(el,'margin-left'))];
	box = el.getBoundingClientRect();
	cPHoverStyle.cssText = 'width:'+(box.right-box.left-2+space[1]+space[3])+'px;'+'height:'+(box.bottom-box.top-2+space[0]+space[2])+'px;left:'+(box.left+db.scrollLeft-space[3])+'px;top:'+(box.top+db.scrollTop-space[0])+'px;display:block';
	cPHoverInnerStyle.cssText = 'width:'+(box.right-box.left-2)+'px;'+'height:'+(box.bottom-box.top-2)+'px;left:'+(space[3]-1)+'px;top:'+(space[0]-1)+'px';
	return el;
}

function chListener(cPMode) { // init/reset eyeDropper / DOM Color-Explorer
	if (cPMode == 0) {
		cPCSS.cssRules[0].style.cssText = 'cursor:default!important';
		cPRanger.style.display='none';
		doCapture=true;
	} else if (cPMode==1){
		// cPDOM.cPOldObj.style.removeProperty('outline');
		cPHoverStyle.display = 'none'
		cPDOM.style.display='none';
		cP.exportColor = null;
	} else {
		cP.exportColor = null;
		cPRanger.style.display='none';
		cPDOM.style.display='none';
		doCapture=true;
		document.removeEventListener('mousemove', doPicker, true);
		// cPDOM.cPOldObj.style.removeProperty('outline');
		cPHoverStyle.display = 'none'
		cPDOM.cPOldObj = document;
		cPCSS.cssRules[0].style.removeProperty('cursor');
	}
}

function checkForDialogues(obj) {
	return (obj.className.indexOf('cP') != 0 && !obj.name) || (obj.name && obj.name.indexOf('cP') != 0) && obj.className.indexOf('cPX') != 0;
}

function getStyle(obj,prop) { // simple version
	return document.defaultView.getComputedStyle(obj,null).getPropertyValue(prop);
}

function X2HEX (rgba) {
	rgba = (rgba+'').replace(/[(^rgba\()]*[^a-fA-F0-9,]*/g,'').split(',');
	a=rgba[3];b=rgba[2];g=rgba[1];r=rgba[0];
	return((r<16?'0':'')+parseInt(r).toString(16)+
				 (g<16?'0':'')+parseInt(g).toString(16)+
				 (b<16?'0':'')+parseInt(b).toString(16)).toUpperCase();
}