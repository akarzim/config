...
function info(msg) {
  if (verbosity >= INFOS) {
    postMessage({ type: "info", message: 'Info: ' + msg });
...
function warn(msg) {
  if (verbosity >= WARNINGS) {
    postMessage({ type: "warning", message: 'Warning: ' + msg });
...
function error(msg) {
  ...
  if (arguments.length > 1) {
    ...
    postMessage({ type: "error", message: logArguments });
    // Join the arguments into a single string for the lines below.
    msg = [].join.call(arguments, ' ');
  } else {
    postMessage({ type: "error", message: 'Error: ' + msg });
  }
  postMessage({ type: "error", message: backtrace() });
