...
function init() {
	...
	chrome.extension.onRequest.addListener(function (request, sender, sendResponse) {
		...
		if (msg == 'openWalkthrough') {
			...
			recordPageInfo(sender.tab, request.domain, request.latency, request.spots);
	...
	// on page completion:
	// - calculate and record page info
	...
	chrome.webRequest.onCompleted.addListener(function (deets) {
		...
		// inject pageInfo analysis script
...
function recordStats(tab_url, bug_url, bug_id, latency) {
	...
	if (!window.censusCache[today]) {
		window.censusCache[today] = {};
	}
	if (!window.censusCache[today][host_with_pathname]) {
		window.censusCache[today][host_with_pathname] = {};
	}

	// record only if current [host_with_pathname]:[web bug] has not already been submitted for today
	if (!window.censusCache[today][host_with_pathname].hasOwnProperty(bug_id)) {
		window.censusCache[today][host_with_pathname][bug_id] = 1;

		census_url = 'http://l.ghostery.com/api/census' +
...
function recordPageInfo(tab, domain, page_latency, ad_spots) {
	if (!window.conf.ghostrank || tab.incognito) {
		return;
	}

	var xhr,
		page_info_url = 'http://l.ghostery.com/api/page/' +
...
// clears any leftover latency (> 5 minutes) and censusCache entry (> yesterday).
function clearEntries() {
	...
	_.each(window.latencies, function (lat, reqId) {
		if ((nowTime - window.latencies[reqId].start_time) > 300000) {
			log("Clearing orphan latencies ...");
			delete window.latencies[reqId];
		}
	});

	_.each(window.censusCache, function (urls, date) {
		if (date != today) {
			log("Clearing old censusCache entries for %s ...", date);
			delete window.censusCache[date];
		}
	});
...
setInterval(clearEntries, 300000); // run every five minutes
