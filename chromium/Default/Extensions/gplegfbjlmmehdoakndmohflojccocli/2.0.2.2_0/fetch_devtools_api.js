...
function fetchDevToolsAPI(real_callback) {
  ...
  if (installDevToolsAlias()) {
    ...
    var script_src = document.referrer.replace(/devtools.html?.*$/,
                                               'devtools_extension_api.js');
    // If the src is http, replace it with https.
    script_src = script_src.replace(/^http:\/\//, 'https://');
    loadExtensionAPI(script_src);
  }
}
